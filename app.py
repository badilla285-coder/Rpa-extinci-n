...
# --- BASE DE DATOS DE USUARIOS (Para tus colegas) ---
# Aquí puedes ir agregando a tus clientes/colegas
USUARIOS_REGISTRADOS = {
    "badilla285@gmail.com": {"nombre": "IGNACIO BADILLA LARA", "pw": "RPA2026"},
    "colega1@pjud.cl": {"nombre": "DEFENSOR ASOCIADO 1", "pw": "LEGAL2026"},
}
...
def check_password():
    if "auth_user" not in st.session_state:
        ...
        st.caption("Inicie sesión para acceder a su Suite Judicial")
        email = st.text_input("Correo electrónico")
        pw = st.text_input("Contraseña", type="password")
        if st.button("Ingresar"):
            if email in USUARIOS_REGISTRADOS and USUARIOS_REGISTRADOS[email]["pw"] == pw:
...
if check_password():
    ...
    with st.sidebar:
        # PUNTO 2: Reloj Chile
        hora_cl = datetime.now() - timedelta(hours=3)
        st.markdown(f"🕒 **Chile: {hora_cl.strftime('%H:%M:%S')}**")
        
        # PUNTO 3: Perfil y Estadísticas
        st.header("👤 Perfil")
        st.write(f"Usuario: **{st.session_state.user_name}**")
        st.write(f"LegalCoins: **{st.session_state.legal_coins}** 🪙")
        
        # Barra de progreso para beneficios
        st.caption("Progreso para recompensa ☕")
        st.progress(min(st.session_state.legal_coins / 500, 1.0))
        
        # Estadísticas del usuario
        st.markdown("---")
        st.subheader("📊 Mis Estadísticas")
        st.metric("Escritos Creados", st.session_state.stats_count)
        
        st.markdown("---")
        st.header("📂 Unir PDFs")
        files = st.file_uploader("Subir documentos", accept_multiple_files=True, type="pdf")
    ...
    if st.button("🚀 GENERAR ESCRITO WORD", use_container_width=True):
        if not imp_nom or not st.session_state.ej_list[0]['rit']:
            ...
            st.session_state.stats_count += 1 # Actualiza estadísticas
            datos = {
