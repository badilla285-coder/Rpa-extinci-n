...
# Paleta de Colores "Nórdico Legal" COMPLETA
COLORS = {
...
# =============================================================================
# 2. MODELOS DE DATOS (SOLUCIÓN ERROR UNTYPED)
# =============================================================================

class Jurisprudencia(rx.Base):
    """Modelo estructurado para resultados de búsqueda. Evita UntypedVarError."""
    rol: str
    tribunal: str
    tipo: str
    contenido: str
...
# =============================================================================
# 3. LÓGICA DE SOPORTE (Helpers fuera del State)
# =============================================================================
...
class GeneradorWord:
    """Clase auxiliar para generar documentos Word"""
...
# =============================================================================
# 4. ESTADO (STATE) - LÓGICA DE NEGOCIO Y DATOS
# =============================================================================
...
class State(rx.State):
    ...
    filtro_tribunal: str = ""
    # CAMBIO CRÍTICO: Lista tipada con el Modelo Jurisprudencia
    ...
    def robustecer_argumentos(self):
        if not self.argumentos_defensa or not GOOGLE_API_KEY: return
        try:
    ...
    def download_docx(self):
        ...
        return rx.download(data=b64_data, filename=f"{self.tipo_recurso.replace(' ', '_')}.docx")
    ...
    # --- FUNCIONES BIBLIOTECA (FIXED: TYPE SAFE) ---
    def buscar_jurisprudencia(self):
        ...
        try:
            emb_resp = genai.embed_content(model="models/text-embedding-004", content=self.busqueda_query, task_type="retrieval_query")
            ...
            if res.data:
                ...
                for doc in res.data:
                    ...
                    if vec_doc and vector:
                        ...
                        # CREAMOS EL OBJETO TIPADO
                        resultados_temp.append(Jurisprudencia(
                ...
                resultados_temp.sort(key=lambda x: x.similarity, reverse=True)
                self.resultados_biblioteca = resultados_temp[:5]
                
                contexto = "\n".join([f"- {d.rol}: {d.contenido[:600]}" for d in self.resultados_biblioteca])
                model = get_generative_model_dinamico()
                resp = model.generate_content(f"Responde jurídicamente a '{self.busqueda_query}' usando SOLO estos fallos:\n{contexto}")
...
def sidebar_item(text, icon, page):
    return rx.button(
        rx.hstack(rx.text(icon), rx.text(text, font_weight="500"), spacing="3"),
...
def login_page():
    return rx.center(
        rx.vstack(
            ...
            rx.text('"Automatización inteligente para defensores: tu tiempo vale."', color=COLORS['slate'], font_style="italic"),
            rx.card(
                rx.vstack(
                    rx.input(placeholder="Correo", on_change=State.set_login_email, style=style_input),
        ...
        width="100vw", height="100vh", bg=COLORS['background']
    )
...
def page_generador():
    return rx.vstack(
        ...
        rx.divider(),
        rx.select(["Prescripción de la Pena", "Extinción Art. 25 ter", "Amparo Constitucional", "Apelación por Quebrantamiento"], value=State.tipo_recurso, on_change=State.set_tipo_recurso, style=style_input),
        rx.grid(
        ...
        rx.button("🧼 Limpiar Campos", on_click=State.clear_form, variant="outline"),
        rx.cond(
        ...
        rx.cond(
            ...
            rx.vstack(
                ...
                rx.button("✨ Robustecer con IA", on_click=State.robustecer_argumentos, bg=COLORS['slate'], color="white"),
            )
        ),
        rx.button("🚀 GENERAR Y DESCARGAR DOCX", on_click=State.download_docx, style=style_button_primary, margin_top="2em"),
        width="100%", padding="2em", spacing="4", max_width="1000px"
    )
...
def page_analista():
    return rx.vstack(
        ...
        rx.radio(["Control de Detención", "Teoría del Caso", "Salidas Alternativas"], value=State.analisis_objetivo, on_change=State.set_analisis_objetivo, direction="row"),
        rx.upload(
            rx.text("Arrastra archivos aquí"), id="upload_analista", multiple=True, accept={"application/pdf": [".pdf"], "image/*": [".png", ".jpg"]},
        ...
        rx.cond(State.is_analyzing, rx.center(rx.spinner(color=COLORS['navy'])), rx.cond(State.analisis_result != "", rx.card(rx.markdown(State.analisis_result), style=style_card, width="100%"))),
        width="100%", padding="2em", spacing="4", max_width="1000px"
    )
...
def page_biblioteca():
    return rx.vstack(
        ...
        rx.cond(
            ...
            rx.vstack(
                rx.cond(State.respuesta_juridica_ia != "", rx.box(rx.heading("⚖️ Respuesta IA", size="3"), rx.markdown(State.respuesta_juridica_ia), bg=COLORS['light_blue'], padding="1.5em", border_radius="10px", width="100%")),
                rx.divider(),
                rx.heading("Fuentes", size="3", color=COLORS['slate']),
                # AQUI SE SOLUCIONA EL ERROR: Iteramos sobre objetos tipados
                rx.foreach(
                    State.resultados_biblioteca,
                    lambda res: rx.card(
                        rx.flex(
                            rx.badge(res.tipo, color_scheme="green"),
                            rx.badge(res.rol, color_scheme="blue"),
                            rx.text(res.tribunal, weight="bold"),
                            spacing="2"
                        ),
                        rx.text(res.contenido, no_of_lines=3, size="2", margin_top="0.5em"),
                        style=style_card, width="100%", margin_bottom="1em"
                    )
                ),
                width="100%"
            )
...
def page_admin():
    return rx.vstack(
        ...
        rx.upload(rx.text("Arrastra PDFs aquí"), id="upload_ingesta", multiple=True, border=f"2px dashed {COLORS['beige']}", padding="3em"),
        ...
        rx.cond(State.is_ingesting, rx.spinner(), rx.text(State.admin_upload_status, color=COLORS['success'])),
        width="100%", padding="2em", spacing="4"
    )
    ...
    return rx.cond(
        ...
        rx.hstack(
            ...
            rx.box(
                ...
                width="100%", height="100vh", overflow_y="auto", bg=COLORS['background']
            ),
            spacing="0"
        ),
        login_page()
